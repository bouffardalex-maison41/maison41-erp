...
// ─── ROUTEUR MULTI-APPLIS ─────────────────────────────────────────────────
// Usage : ?app=gerant (défaut) | ouvrier | vendeuse | compta
// Chaque appli est un fichier HTML distinct dans le projet GAS.
// Tant que les 4 fichiers ne sont pas créés, tout tombe sur Index (= monolithe actuel).
function doGet(e) {
  var app = (e && e.parameter && e.parameter.app) ? e.parameter.app.toLowerCase() : 'gerant';
  var appMap = {
    gerant:   'Gerant',    // Gerant.html  — accent violet #534AB7 — toutes vues
    ouvrier:  'Ouvrier',   // Ouvrier.html — accent bleu  #185FA5 — Labo/HACCP/Autoclaves/Traçabilité
    vendeuse: 'Vendeuse',  // Vendeuse.html — accent vert  #1a7a4a — Ventes/Marchés/Clients/Bons/Prix
    compta:   'Compta'     // Compta.html  — accent orange #d4520a — Finance/Comptabilité
  };
  var fileName = appMap[app] || 'Index';
  var t = HtmlService.createTemplateFromFile(fileName);
  t.appRole = app; // injecté dans le template comme <?= appRole ?>
  try {
    // Gérant et Vendeuse : toutes les données
    // Ouvrier : appData + prodData  |  Compta : appData + ventesData uniquement
    try { t.initialAppData = safeJson(getAllAppData()); }
    catch(e1){ Logger.log('doGet appData error: '+e1.message); t.initialAppData = 'null'; }

    try {
      if(app === 'compta'){
        t.initialProdData = 'null';
      } else {
        t.initialProdData = safeJson(getProductionData());
      }
    }
  ...
  Logger.log('doGet app='+app+' file='+fileName+' appData='+t.initialAppData.length);
  return t.evaluate()
    .setTitle('MAISON 41 — ERP · ' + (app.charAt(0).toUpperCase()+app.slice(1)))
    .addMetaTag('viewport','width=device-width,initial-scale=1,maximum-scale=1')
    .setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
}

// ─── CACHE FRAÎCHEUR (jeton LastWrite) ────────────────────────────────────
// Chaque appli cliente compare localStorage.getItem('lastFetch_'+app)
// avec getLastWrite() avant de réutiliser son cache.
function getLastWrite(){
  try{
    var sh=SS().getSheetByName('Config');
    if(!sh) return '';
    var d=sh.getDataRange().getValues();
    for(var i=0;i<d.length;i++) if(String(d[i][0]).trim()==='LastWrite') return String(d[i][1]||'');
    return '';
  }catch(e){ return ''; }
}
function _setLastWrite(){
  try{
    var sh=SS().getSheetByName('Config');if(!sh) return;
    var d=sh.getDataRange().getValues();
    for(var i=0;i<d.length;i++){
      if(String(d[i][0]).trim()==='LastWrite'){sh.getRange(i+1,2).setValue(new Date().toISOString());return;}
    }
    // Clé absente → ajouter
    var row=firstEmptyRow(sh,1,1);
    sh.getRange(row,1,1,2).setValues([['LastWrite',new Date().toISOString()]]);
  }catch(e){ Logger.log('_setLastWrite:'+e.message); }
}
...
function getDashboardData(){
  try{
    ...
    var shV=ss.getSheetByName('Ventes'),caM=0,nbV=0,caA=0,caMPrec=0;
    if(shV){var vd=shV.getDataRange().getValues();for(var i=4;i<vd.length;i++){var d=vd[i][0];if(!(d instanceof Date))continue;var n=parseFloat(vd[i][10])||0;if(d.getFullYear()===annee){caA+=n;if(d.getMonth()+1===mois){caM+=n;nbV++;}}
      ...
      if(d.getFullYear()===annee-1&&d.getMonth()+1===mois){caMPrec+=n;}
    }}
    r.caMonth=caM;r.caAnnee=caA;r.nbVentes=nbV;r.panierMoy=nbV>0?caM/nbV:0;
    r.caAnneePrec=caMPrec; // CA du même mois N-1
...
function getAllAppData(){
  try{
    ...
    r.recettes=readSheet('Recettes').filter(function(x){return x['Ref.']||x['Ref'];}).map(function(x){
      return{ref:x['Ref.']||x['Ref'],nom:x['Nom commercial']||x['Nom']||'',gamme:x['Gamme']||'',format:x['Format principal']||x['Format Weck']||'',tempCCP1:parseFloat(x['Temp_CCP1']||x['Temp CCP1'])||115,dureeBareme:parseFloat(x['Duree bareme (min)']||x['Temps (h)'])||0,machines:x['Machines_utilisees']||x['Machines utilisees']||'',statut:x['Statut']||'Actif'};
    });
...
function saveAutoclave(record){
  try{
    ...
    var row=firstEmptyRow(sh,1,4),d=record.date?new Date(record.date):new Date();
    // Col 1-11 : date, autoclave, numLot, recette, format, nbBocaux, tempMax, duree, coût (formule), statut (formule), notes
    // Col 12 : pression (bar) — L1-P1
    sh.getRange(row,1,1,12).setValues([[d,record.autoclave||'',record.numLot||'',record.recette||'',record.format||'',parseInt(record.nbBocaux)||0,parseFloat(record.tempMax)||0,parseInt(record.duree)||0,'','',record.notes||'',parseFloat(record.pression)||0]]);
    ...
    // NC auto si durée réelle < durée barème recette
    var dureeReelle=parseInt(record.duree)||0;
    var dureeBareme=parseInt(record.dureeBareme)||0;
    _setLastWrite();
    if(dureeBareme>0&&dureeReelle<dureeBareme){
      saveHACCP({date:Utilities.formatDate(d,'Europe/Paris','yyyy-MM-dd'),type:'CCP1 — Durée stérilisation',description:'NC AUTO — Lot '+(record.numLot||'')+' — '+(record.recette||'')+' — Durée='+dureeReelle+'min (barème ≥'+dureeBareme+'min)',valeur:dureeReelle,limite:dureeBareme,action:'Vérifier cycle autoclave — lot en quarantaine',responsable:record.operateur||''});
    }
    return{success:true,row:row};
...
// ─── EMAIL BC ─────────────────────────────────────────────────────────────
function _buildBCHtml(numBC,bc,lignes,totHTnet,totTVA,totTTC){
  var cfg=readConfig();
  var rs=cfg['Raison sociale']||'Maison 41';
  var addr=cfg['Adresse']||'Cognac, France';
  var rows=lignes.map(function(l,i){
    var qte=parseInt(l.qte)||0;var pu=parseFloat(l.puHT)||0;
    return'<tr style="border-bottom:1px solid #eee"><td style="padding:8px 12px">'+escHtml(l.recette||l.nom||'')+'</td><td style="padding:8px 12px;color:#666">'+escHtml(l.format||'—')+'</td><td style="padding:8px 12px;text-align:right">'+qte+'</td><td style="padding:8px 12px;text-align:right">'+pu.toFixed(2)+' €</td><td style="padding:8px 12px;text-align:right;font-weight:600">'+(qte*pu).toFixed(2)+' €</td></tr>';
  }).join('');
  var remise=parseFloat(bc.remise||0);
  var remiseRow=remise>0?'<tr><td colspan="4" style="padding:6px 12px;text-align:right;color:#888">Remise '+remise+'%</td><td style="padding:6px 12px;text-align:right;color:#d4520a">−'+(lignes.reduce(function(s,l){return s+(parseInt(l.qte)||0)*(parseFloat(l.puHT)||0);},0)*remise/100).toFixed(2)+' €</td></tr>':'';
  var dateStr=bc.date?Utilities.formatDate(new Date(bc.date),'Europe/Paris','dd/MM/yyyy'):Utilities.formatDate(new Date(),'Europe/Paris','dd/MM/yyyy');
  var dlStr=bc.dateLivraison?Utilities.formatDate(new Date(bc.dateLivraison),'Europe/Paris','dd/MM/yyyy'):'À définir';
  return'<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="margin:0;padding:0;font-family:Arial,sans-serif;background:#f5f1e8">'+
  ...
  '<div style="background:#534AB7;padding:24px 28px;color:#fff"><div style="font-size:22px;font-weight:700">'+escHtml(rs)+'</div><div style="font-size:13px;opacity:.8;margin-top:4px">'+escHtml(addr)+'</div></div>'+
  '<div style="padding:24px 28px">'+
  '<h2 style="margin:0 0 4px;font-size:20px;color:#1a1a2e">Bon de Commande '+escHtml(numBC)+'</h2>'+
  '<p style="margin:0 0 20px;color:#666;font-size:14px">Date : '+dateStr+' · Livraison souhaitée : '+dlStr+'</p>'+
  '<p style="margin:0 0 8px;font-size:14px"><strong>Client :</strong> '+escHtml(bc.client||'')+'</p>'+
  (bc.notes?'<p style="margin:0 0 16px;font-size:13px;color:#888">'+escHtml(bc.notes)+'</p>':'')+
  '<table style="width:100%;border-collapse:collapse;margin-top:12px"><thead><tr style="background:#f5f1e8"><th style="padding:8px 12px;text-align:left;font-size:13px">Produit</th><th style="padding:8px 12px;text-align:left;font-size:13px">Format</th><th style="padding:8px 12px;text-align:right;font-size:13px">Qté</th><th style="padding:8px 12px;text-align:right;font-size:13px">PU HT</th><th style="padding:8px 12px;text-align:right;font-size:13px">Total HT</th></tr></thead><tbody>'+rows+remiseRow+'</tbody><tfoot>'+
  '<tr style="border-top:2px solid #534AB7"><td colspan="4" style="padding:10px 12px;text-align:right;font-weight:600">Total HT</td><td style="padding:10px 12px;text-align:right;font-weight:700;color:#534AB7">'+totHTnet.toFixed(2)+' €</td></tr>'+
  '<tr><td colspan="4" style="padding:4px 12px;text-align:right;color:#888;font-size:13px">TVA</td><td style="padding:4px 12px;text-align:right;color:#888;font-size:13px">'+totTVA.toFixed(2)+' €</td></tr>'+
  '<tr style="background:#534AB7"><td colspan="4" style="padding:10px 12px;text-align:right;color:#fff;font-weight:700">Total TTC</td><td style="padding:10px 12px;text-align:right;color:#fff;font-weight:700;font-size:16px">'+totTTC.toFixed(2)+' €</td></tr>'+
  '</tfoot></table>'+
  (bc.notesConditions?'<p style="margin:16px 0 0;font-size:13px;color:#888;border-top:1px solid #eee;padding-top:12px">'+escHtml(bc.notesConditions)+'</p>':'')+
  '</div><div style="background:#f5f1e8;padding:14px 28px;font-size:12px;color:#888;text-align:center">'+escHtml(rs)+' — '+escHtml(addr)+'</div></div></body></html>';
}

function escHtml(s){return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}

function sendBCEmail(bcData){
  // Appelé depuis saveBC_form si bc.sendEmail===true
  // bcData = {numBC, client, emailDest, lignes[], remise, totHTnet, totTVA, totTTC, ...}
  try{
    var cfg=readConfig();
    var dest=String(bcData.emailDest||'').trim();
    if(!dest||dest.indexOf('@')<0) return{success:false,error:'Email destinataire invalide'};
    var rs=cfg['Raison sociale']||'Maison 41';
    var html=_buildBCHtml(bcData.numBC||'BC',bcData,bcData.lignes||[],
      parseFloat(bcData.totHTnet||0),parseFloat(bcData.totTVA||0),parseFloat(bcData.totTTC||0));
    MailApp.sendEmail({
      to:dest,
      subject:'['+rs+'] Bon de commande '+escHtml(bcData.numBC||'')+'',
      htmlBody:html,
      noReply:false
    });
    return{success:true};
  }catch(e){Logger.log('sendBCEmail:'+e.message);return{success:false,error:e.message};}
}
...
function sendMonthlyReport(){
  try{
    var cfg=readConfig();
    var destGerant=String(cfg['Email rapport mensuel']||cfg['Email contact gerant']||'').trim();
    var destCompta=String(cfg['Email comptable']||'').trim();
    var dests=[destGerant,destCompta].filter(function(e){return e&&e.indexOf('@')>0;});
    if(!dests.length){Logger.log('sendMonthlyReport: aucun email configuré');return{success:false,error:'Aucun email configuré dans Config'};}
    var rs=cfg['Raison sociale']||'Maison 41';
    var now=new Date();
    var moisPrec=new Date(now.getFullYear(),now.getMonth()-1,1);
    var moisLabel=Utilities.formatDate(moisPrec,'Europe/Paris','MMMM yyyy');
    // Lecture données
    var finData=getFinanceData();
    var dashData=getDashboardData();
    var lots=readSheet('Production')||[];
    var haccp=readSheet('HACCP')||[];
    var ventes=readSheet('Ventes')||[];
    var monthStr=Utilities.formatDate(moisPrec,'Europe/Paris','yyyy-MM');
    var lotsMonth=lots.filter(function(r){return String(r['Date']||'').startsWith(monthStr);}).length;
    var ventesMonth=ventes.filter(function(r){return String(r['Date']||r['date']||'').startsWith(monthStr);});
    var caMonth=ventesMonth.reduce(function(s,v){return s+parseFloat(v['Total TTC']||v['TotalTTC']||v['CA']||0);},0);
    var haccpNC=haccp.filter(function(r){return String(r['Date']||'').startsWith(monthStr)&&String(r['Type']||'').indexOf('CCP')>=0;}).length;
    var html='<!DOCTYPE html><html><head><meta charset="utf-8"></head><body style="font-family:Arial,sans-serif;background:#f5f1e8;margin:0;padding:0">'+
    '<div style="max-width:620px;margin:24px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.08)">'+
    '<div style="background:#534AB7;padding:24px 28px;color:#fff">'+
    '<div style="font-size:20px;font-weight:700">📊 Rapport mensuel — '+escHtml(moisLabel)+'</div>'+
    '<div style="opacity:.8;font-size:13px;margin-top:4px">'+escHtml(rs)+'</div></div>'+
    '<div style="padding:24px 28px">'+
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px">'+
    _kpiCard('💰 CA TTC',caMonth.toFixed(2)+' €','#0F6E56')+
    _kpiCard('📦 Lots produits',String(lotsMonth),'#185FA5')+
    _kpiCard('🛒 Ventes',String(ventesMonth.length),'#534AB7')+
    _kpiCard('⚠️ NC HACCP CCP',String(haccpNC),haccpNC>0?'#c0392b':'#0F6E56')+
    '</div>'+
    '<p style="font-size:13px;color:#888;border-top:1px solid #eee;padding-top:16px;margin:0">Rapport généré automatiquement par MAISON41 ERP le '+Utilities.formatDate(now,'Europe/Paris','dd/MM/yyyy à HH:mm')+'</p>'+
    '</div></div></body></html>';
    dests.forEach(function(dest){
      MailApp.sendEmail({to:dest,subject:'['+rs+'] Rapport mensuel '+moisLabel,htmlBody:html});
    });
    return{success:true,destinataires:dests,mois:moisLabel};
  }catch(e){Logger.log('sendMonthlyReport:'+e.message);return{success:false,error:e.message};}
}

function _kpiCard(label,val,color){
  return'<div style="background:#f9f8ff;border-radius:8px;padding:16px;border-left:4px solid '+color+'">'+
  '<div style="font-size:11px;color:#888;text-transform:uppercase;letter-spacing:.5px">'+label+'</div>'+
  '<div style="font-size:24px;font-weight:700;color:'+color+';margin-top:6px">'+val+'</div></div>';
}
...
function testNtfyPush(){
  try{
    ...
    if(!topic) return{success:false,error:'Clé "ntfy Topic" absente dans Config'};
    sendPush('[MAISON41] 🔔 Test push OK — ntfy.sh configuré','default');
    return{success:true,topic:topic};
  }catch(e){Logger.log('testNtfyPush:'+e.message);return{success:false,error:e.message};}
}
...
function getFinanceData(){
  try{
    ...
    // N-1 : colonne 'CA N-1' dans la même feuille OU feuille dédiée 'Previsionnel N-1'
    var shN1=SS().getSheetByName('Previsionnel N-1')||SS().getSheetByName('Prévisionnel N-1');
    if(shN1){
      r.previsionnel_n1=readSheet('Previsionnel N-1').filter(function(x){return x['Mois'];});
    } else {
      // Fallback : lire colonne 'CA N-1' dans la feuille Prévisionnel
      r.previsionnel_n1=r.previsionnel.filter(function(x){
        return x['CA N-1']!==undefined&&x['CA N-1']!==''&&x['CA N-1']!==null;
      });
    }
  ...
  try{
    ...
    return readSheet('Recettes').filter(function(x){return x['Ref.']||x['Ref'];}).map(function(r){
      ...
      return{
        ...
        // tempCCP1 lu depuis Fiches Techniques col 22 ou Config par défaut
        tempCCP1:parseFloat(ft['Bareme steril.']||ft['T° bareme (°C)']||ft['Temp CCP1'])||115,
        dureeBareme:parseFloat(r['Duree bareme (min)']||r['Temps (h)'])||0,
...
// ─── MIGRATION BARÈMES ────────────────────────────────────────────────────
// À exécuter UNE FOIS depuis l'éditeur GAS pour initialiser les barèmes existants.
// Préremplir avec 115°C / 25 min par défaut sur toutes les recettes sans barème.
// Modifier les valeurs dans les cellules Sheets ensuite recette par recette.
function migrateBaremes(){
  try{
    var shR=SS().getSheetByName('Recettes');
    var shFT=SS().getSheetByName('Fiches Techniques');
    if(!shR||!shFT){Logger.log('migrateBaremes: feuilles manquantes');return{success:false};}
    var dR=shR.getDataRange().getValues();
    var dFT=shFT.getDataRange().getValues();
    // Recettes : en-tête ligne 3 (index 2), données à partir index 3
    // Col 7 = Duree bareme (min)
    var updR=0;
    for(var i=3;i<dR.length;i++){
      if(!dR[i][0])continue; // ligne vide
      if(!dR[i][6]||dR[i][6]===''||dR[i][6]===0){
        shR.getRange(i+1,7).setValue(25); // 25 min par défaut
        updR++;
      }
    }
    // Fiches Techniques : en-tête ligne 4 (index 3), données à partir index 4
    // Col 22 = T° bareme (°C), col 25 = Duree bareme (min)
    var updFT=0;
    for(var j=4;j<dFT.length;j++){
      if(!dFT[j][0])continue;
      if(!dFT[j][21]||dFT[j][21]===''||dFT[j][21]===0){
        shFT.getRange(j+1,22).setValue(115);
        updFT++;
      }
      if(!dFT[j][24]||dFT[j][24]===''||dFT[j][24]===0){
        shFT.getRange(j+1,25).setValue(25);
        updFT++;
      }
    }
    Logger.log('migrateBaremes: '+updR+' recettes + '+updFT+' FT mises à jour (défaut 115°C/25min)');
    return{success:true,recettesUpdated:updR,ftUpdated:updFT};
  }catch(e){Logger.log('migrateBaremes:'+e.message);return{success:false,error:e.message};}
}
...
function saveRecette(recette){
  try{
    ...
    sh.getRange(row,1,1,11).setValues([[recette.ref||'',recette.nom||'',recette.gamme||'',recette.formatPrincipal||'',parseFloat(recette.rendement)||'',parseFloat(recette.poidsBrut)||'',parseInt(recette.dureeBareme)||'',recette.statut||'Actif','','',now]]);
    _saveDetailRecette(recette.ref,recette.ingredients||[]);
    // FIX #10 : passer tempCCP1, ddm, allergens, dureeBareme à _saveFicheTech
    _saveFicheTech(recette.ref,recette.nom,recette.nutri||{},{tempCCP1:recette.tempCCP1||115,dureeBareme:parseInt(recette.dureeBareme)||0,ddm:recette.ddm||24,allergens:recette.allergens||'',traces:recette.traces||''});
    _setLastWrite();
    return{success:true,row:row};
  }catch(e){Logger.log('saveRecette:'+e.message);return{success:false,error:e.message};}
}

// ─── SNAPSHOT RECETTE dans "Historique Recettes" ──────────────────────────
// Appelé avant tout updateRecette pour conserver l'état antérieur.
// Colonnes : Date snapshot | Ref | Nom | Gamme | Format | Duree bareme | T°CCP1 | Statut | Auteur
function _snapshotRecette(ref,auteur){
  try{
    var ss=SS();
    var shH=ss.getSheetByName('Historique Recettes');
    if(!shH){
      shH=ss.insertSheet('Historique Recettes');
      var hdrs=['Date snapshot','Ref.','Nom commercial','Gamme','Format principal','Duree bareme (min)','Temp CCP1 (C)','Statut','Auteur'];
      shH.getRange(1,1,1,hdrs.length).setValues([hdrs]);
      shH.getRange(1,1,1,hdrs.length).setBackground('#0d0d0d').setFontColor('#f0c060').setFontWeight('bold');
      shH.setFrozenRows(1);
      shH.autoResizeColumns(1,hdrs.length);
    }
    // Lire la version actuelle depuis Recettes
    var shR=ss.getSheetByName('Recettes');
    if(!shR) return;
    var d=shR.getDataRange().getValues();
    var now=Utilities.formatDate(new Date(),'Europe/Paris','yyyy-MM-dd HH:mm:ss');
    for(var i=3;i<d.length;i++){
      if(String(d[i][0]).trim()===String(ref).trim()){
        var row=shH.getLastRow()+1;
        shH.getRange(row,1,1,9).setValues([[
          now,
          d[i][0]||'',  // Ref.
          d[i][1]||'',  // Nom commercial
          d[i][2]||'',  // Gamme
          d[i][3]||'',  // Format principal
          d[i][6]||'',  // Duree bareme (min)
          '',            // T°CCP1 (lu depuis Fiches Techniques — laisser vide ici)
          d[i][7]||'',  // Statut
          auteur||'système'
        ]]);
        Logger.log('_snapshotRecette: snapshot créé pour '+ref+' à '+now);
        return;
      }
    }
  }catch(e){Logger.log('_snapshotRecette: '+e.message);}
}

// ─── HISTORIQUE RECETTES — lecture ────────────────────────────────────────
// Retourne les snapshots d'une recette (ou tous si ref omis)
function getHistoriqueRecettes(ref){
  try{
    var rows=readSheet('Historique Recettes');
    if(ref) rows=rows.filter(function(r){return String(r['Ref.']||r['Ref']||'').trim()===String(ref).trim();});
    // Plus récents en premier
    return{success:true,data:rows.reverse().slice(0,50)};
  }catch(e){Logger.log('getHistoriqueRecettes:'+e.message);return{success:false,error:e.message,data:[]};}
}


function getStockFinis(){
  try{
    var rows=readSheet('Planning');
    // Stock finis = lots Validés non encore vendus
    var finis=rows.filter(function(r){
      var s=String(r['Statut']||r['Status']||'').toLowerCase();
      return s==='validé'||s=='valide'||s==='terminé'||s==='termine';
    }).map(function(r){
      return{
        numLot:r['N° Lot']||r['Num Lot']||r['NumLot']||'',
        recette:r['Recette']||'',
        gamme:r['Gamme']||'',
        format:r['Format']||'',
        qte:parseFloat(r['Qté finie']||r['Qte finie']||r['Qte']||0),
        unite:r['Unité']||r['Unite']||'bocaux',
        dateProd:r['Date']||'',
        ddm:r['DDM']||'',
        statut:r['Statut']||r['Status']||''
      };
    });
    return{success:true,data:finis};
  }catch(e){
    Logger.log('getStockFinis:'+e.message);
...
function updateRecette(recette){
  try{
    var sh=SS().getSheetByName('Recettes');if(!sh) return{success:false,error:'Feuille Recettes introuvable'};
    // ── VERSIONING : snapshot avant écrasement ──
    _snapshotRecette(recette.ref, recette._auteur||'Gérant');
    var d=sh.getDataRange().getValues();
    ...
    for(var i=3;i<d.length;i++){
      if(String(d[i][0]).trim()===String(recette.ref||'').trim()){
        ...
        sh.getRange(i+1,7).setValue(parseInt(recette.dureeBareme)||'');
        sh.getRange(i+1,8).setValue(recette.statut||'Actif');
        _saveDetailRecette(recette.ref,recette.ingredients||[]);
        // FIX #10 : passer tempCCP1, ddm, allergens, dureeBareme
        _saveFicheTech(recette.ref,recette.nom,recette.nutri||{},{tempCCP1:recette.tempCCP1||115,dureeBareme:parseInt(recette.dureeBareme)||0,ddm:recette.ddm||24,allergens:recette.allergens||'',traces:recette.traces||''});
        _setLastWrite();
        return{success:true};
      }
...
function _saveFicheTech(ref,nom,nutri,recetteData){
  ...
  try{
    ...
    // Barème stérilisation T° (col 22), durée (col 25) et DDM mois (col 23)
    if(rd.tempCCP1) sh.getRange(target,22).setValue(parseFloat(rd.tempCCP1)||115);
    if(rd.dureeBareme) sh.getRange(target,25).setValue(parseInt(rd.dureeBareme)||0);
...
function initConfigDefaults(){
  var defaults=[
    ...
    ['Email contact gerant',''],
    ['Email comptable',''],
    ['Email rapport mensuel',''],
    ['Objectif CA hebdo (EUR)',''],
    ['Seuil client fidele (EUR)','150'],
    ['TVA taux standard (%)','5.5'],
    ['ntfy Topic',''],
    ['ntfy Priority','high']
  ];
...
function updatePrevisionnel(items){
  try{
    var sh=SS().getSheetByName('Previsionnel')||SS().getSheetByName('Prévisionnel');
...
// ─── savePrevRow : sauvegarde d'une ligne prévisionnelle individuelle ──────────
// Appelé depuis Compta.html : savePrevRow({mois, canal, reel})
function savePrevRow(obj){
  try{
    if(!obj||!obj.mois) return{success:false,error:'Mois requis'};
    var sh=SS().getSheetByName('Previsionnel')||SS().getSheetByName('Prévisionnel');
    if(!sh) return{success:false,error:'Feuille Previsionnel introuvable'};
    var d=sh.getDataRange().getValues();
    // Détection en-tête (même logique que readSheet)
    var hIdx=0,bestScore=0;
    for(var i=0;i<Math.min(d.length,10);i++){
      var sc=0;
      for(var j=0;j<d[i].length;j++){if(d[i][j]&&typeof d[i][j]==='string')sc++;}
      if(sc>bestScore){bestScore=sc;hIdx=i;}
    }
    var h=d[hIdx].map(function(x){return String(x).trim().toLowerCase();});
    var colM=h.findIndex(function(k){return k==='mois';});
    var colC=h.findIndex(function(k){return k.includes('canal')||k.includes('poste');});
    var colCR=h.findIndex(function(k){return k.includes('ca')&&(k.includes('reel')||k.includes('réel'));});
    if(colM<0) return{success:false,error:'Colonne Mois introuvable'};
    // Chercher la ligne correspondant à mois + canal
    for(var i=hIdx+1;i<d.length;i++){
      var rowMois=String(d[i][colM]||'').trim();
      var rowCanal=colC>=0?String(d[i][colC]||'').trim():'';
      var moisMatch=rowMois===String(obj.mois||'').trim();
      var canalMatch=colC<0||(rowCanal===String(obj.canal||'').trim())||rowCanal===''||obj.canal===undefined;
      if(moisMatch&&canalMatch){
        if(colCR>=0) sh.getRange(i+1,colCR+1).setValue(parseFloat(obj.reel)||0);
        _setLastWrite();
        return{success:true,row:i+1};
      }
    }
    // Ligne non trouvée — créer une nouvelle entrée
    if(colM>=0){
      var newRow=hIdx+1+d.length;
      if(colM>=0) sh.getRange(d.length+1,colM+1).setValue(obj.mois||'');
      if(colC>=0) sh.getRange(d.length+1,colC+1).setValue(obj.canal||'');
      if(colCR>=0) sh.getRange(d.length+1,colCR+1).setValue(parseFloat(obj.reel)||0);
      _setLastWrite();
      return{success:true,created:true,row:d.length+1};
    }
    return{success:false,error:'Structure feuille incompatible'};
  }catch(e){Logger.log('savePrevRow:'+e.message);return{success:false,error:e.message};}
}

// ─── savePrevisionnel : batch-save de tous les items prévisionnels ─────────────
// Appelé depuis Compta.html : savePrevisionnel(items[])
// Wrapper de updatePrevisionnel avec mapping des champs Compta → updatePrevisionnel
function savePrevisionnel(items){
  try{
    if(!Array.isArray(items)||!items.length) return{success:false,error:'Aucune donnée'};
    // Mapper les champs Compta {Mois, Canal/Poste, CA Prévisionnel, CA Réel}
    // vers le format updatePrevisionnel {mois, caPrev, caReal}
    var mapped=items.map(function(r){
      return{
        mois:r['Mois']||r['mois']||'',
        canal:r['Canal']||r['Poste']||r['canal']||'',
        caPrev:parseFloat(r['CA Prévisionnel']||r['Prev']||r['caPrev']||0),
        caReal:parseFloat(r['CA Réel']||r['Reel']||r['caReal']||0)
      };
    });
    return updatePrevisionnel(mapped);
  }catch(e){Logger.log('savePrevisionnel:'+e.message);return{success:false,error:e.message};}
}
...
// ─── PHOTO PRODUIT FINI ──────────────────────────────────────────────────────
// Sauvegarde une photo produit fini dans Drive et note l'URL dans la feuille Production
function saveProductPhoto(data){
  try{
    var numLot=String(data.numLot||'').trim();
    var b64=String(data.base64||'');
    var mime=String(data.mimeType||'image/jpeg');
    if(!numLot||!b64) return{success:false,error:'Paramètres manquants'};
    // Créer ou retrouver le dossier Drive
    var root=DriveApp.getRootFolder();
    var maison41=getOrCreateDriveFolder(root,'MAISON41 — Traçabilité');
    var photosF=getOrCreateDriveFolder(maison41,'Photos Produits Finis');
    var ext=mime.indexOf('png')>=0?'.png':'.jpg';
    var fname='PRODUIT_'+numLot+'_'+Utilities.formatDate(new Date(),'Europe/Paris','yyyyMMdd_HHmm')+ext;
    var blob=Utilities.newBlob(Utilities.base64Decode(b64),mime,fname);
    var file=photosF.createFile(blob);
    file.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);
    var url=file.getUrl();
    // Écrire l'URL dans la colonne "URL Photo Produit" de la feuille Production
    var sh=SS().getSheetByName('Production');
    if(sh){
      var vals=sh.getDataRange().getValues();
      var headers=vals[0].map(function(h){return String(h).trim();});
      var lotCol=headers.indexOf('N Lot (auto)');if(lotCol<0)lotCol=headers.indexOf('N Lot');
      var photoCol=headers.indexOf('URL Photo Produit');
      if(photoCol<0){
        // Créer la colonne si absente
        photoCol=headers.length;
        sh.getRange(1,photoCol+1).setValue('URL Photo Produit');
      }
      if(lotCol>=0){
        for(var i=1;i<vals.length;i++){
          if(String(vals[i][lotCol]||'').trim()===numLot){
            sh.getRange(i+1,photoCol+1).setValue(url);
            break;
          }
        }
      }
  ...
  }catch(e){Logger.log('saveProductPhoto:'+e.message);return{success:false,error:e.message};}
}

// ─── DOSSIER DDPP ────────────────────────────────────────────────────────────
// Compile un dossier DDPP dans un Google Doc : agréments, HACCP, traçabilité, réclamations
function exportDossierDDPP(){
  try{
    var cfg=readConfig();
    var rs=cfg['Raison sociale']||'Maison 41';
    var siret=cfg['SIRET']||'—';
    var agree=cfg['Numero agrement DDPP']||'—';
    var addr=cfg['Adresse']||'Cognac, France';
    var now=new Date();
    var dateStr=Utilities.formatDate(now,'Europe/Paris','dd/MM/yyyy');
    // Récupérer les données clés
    var haccp=readSheet('HACCP')||[];
    var trac=readSheet('Traçabilité')||[];
    var rec=readSheet('Réclamations')||readSheet('Reclamations')||[];
    var lots=readSheet('Production')||[];
    var nLots=lots.length;
    var nHaccp=haccp.length;
    var nTrac=trac.length;
    var nRec=rec.length;
    var nRecCrit=rec.filter(function(r){return String(r['Gravité']||r['Gravite']||'').indexOf('Critique')>=0;}).length;
    // Créer le Google Doc
    var docName='MAISON41 — Dossier DDPP — '+Utilities.formatDate(now,'Europe/Paris','yyyy-MM-dd');
    var doc=DocumentApp.create(docName);
    var body=doc.getBody();
    // En-tête
    body.appendParagraph(rs+' — Dossier DDPP').setHeading(DocumentApp.ParagraphHeading.TITLE);
    body.appendParagraph('Généré le '+dateStr+' · '+addr).setHeading(DocumentApp.ParagraphHeading.SUBTITLE);
    body.appendParagraph('');
    // 1. Identification
    body.appendParagraph('1. Identification de l\'établissement').setHeading(DocumentApp.ParagraphHeading.HEADING1);
    var tbl1=body.appendTable([
      ['Raison sociale',rs],['SIRET',siret],['Numéro agrément DDPP',agree],['Adresse',addr]
    ]);
    tbl1.setBorderWidth(1);
    body.appendParagraph('');
    // 2. Production
    body.appendParagraph('2. Activité de production').setHeading(DocumentApp.ParagraphHeading.HEADING1);
    body.appendParagraph('Nombre de lots enregistrés : '+nLots);
    body.appendParagraph('Produits : Rillettes, Pâtés, Mousses, Terrines, Cassoulet, Desserts (conserves appertisées).');
    body.appendParagraph('Réglementation applicable : CE 852/2004 (hygiène), UE 178/2002 (traçabilité), INCO 1169/2011 (allergènes).');
    body.appendParagraph('');
    // 3. HACCP
    body.appendParagraph('3. Plan HACCP — CCP enregistrés').setHeading(DocumentApp.ParagraphHeading.HEADING1);
    body.appendParagraph('Nombre d\'enregistrements HACCP : '+nHaccp);
    body.appendParagraph('CCP1 : Stérilisation autoclave — T° min '+String(cfg['Autoclave temperature minimale (C)']||'115')+'°C — Fo ≥ 3 min recommandé.');
    if(nHaccp>0){
      var hRows=[['Date','Type CCP','Valeur mesurée','Limite','Statut']];
      haccp.slice(-20).forEach(function(r){
        hRows.push([String(r['Date']||'').split('T')[0]||'—',String(r['Type CCP']||r['Type']||''),String(r['Valeur mesurée']||r['Valeur Mesuree']||''),String(r['Limite']||''),String(r['Statut']||r['status']||'')]);
      });
      var tHaccp=body.appendTable(hRows);
      tHaccp.setBorderWidth(1);
    }
    body.appendParagraph('');
    // 4. Traçabilité
    body.appendParagraph('4. Traçabilité amont (UE 178/2002)').setHeading(DocumentApp.ParagraphHeading.HEADING1);
    body.appendParagraph('Nombre d\'entrées de traçabilité MP : '+nTrac);
    body.appendParagraph('Chaque lot est tracé avec : N° lot fournisseur, fournisseur, DLC fournisseur, quantité consommée, URL photo étiquette.');
    body.appendParagraph('');
    // 5. Réclamations
    body.appendParagraph('5. Réclamations & non-conformités').setHeading(DocumentApp.ParagraphHeading.HEADING1);
    body.appendParagraph('Nombre de réclamations : '+nRec+' (dont critiques : '+nRecCrit+')');
    if(nRecCrit>0){
      body.appendParagraph('⚠️ Réclamations critiques — traitement prioritaire sous 24h requis (danger santé).').setBold(true);
    } else {
      body.appendParagraph('Aucune réclamation critique ouverte.');
    }
    body.appendParagraph('');
    // Pied
    body.appendParagraph('Document généré automatiquement par MAISON41 ERP. À compléter et signer avant transmission à la DDPP.')
      .setItalic(true);
    doc.saveAndClose();
    var docFile=DriveApp.getFileById(doc.getId());
    docFile.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);
    return{success:true,url:docFile.getUrl(),nom:docName,nLots:nLots,nHaccp:nHaccp,nTrac:nTrac,nRec:nRec};
  }catch(e){Logger.log('exportDossierDDPP:'+e.message);return{success:false,error:e.message};}
}
...
// ─── PMS — SAUVEGARDE RUBRIQUE ────────────────────────────────────────────
// obj = { sectionOld, section, contenu, statut, auteur }
// sectionOld = titre original (pour mise à jour) ou absent (pour création)
function savePMS(obj){
  try{
    var ss=SS();
    var sh=ss.getSheetByName('PMS');
    if(!sh){
      sh=ss.insertSheet('PMS');
      var hdrs=['Section','Contenu','Date MAJ','Statut','Auteur'];
      sh.getRange(1,1,1,hdrs.length).setValues([hdrs]);
      sh.getRange(1,1,1,hdrs.length).setBackground('#0d0d0d').setFontColor('#f0c060').setFontWeight('bold');
      sh.setFrozenRows(1);sh.autoResizeColumns(1,hdrs.length);
      Logger.log('savePMS: feuille PMS créée');
    }
    var now=Utilities.formatDate(new Date(),'Europe/Paris','yyyy-MM-dd HH:mm');
    var d=sh.getDataRange().getValues();
    var h=d[0].map(function(x){return String(x).trim();});
    var cSec=h.indexOf('Section')||0;
    var cCont=h.indexOf('Contenu');if(cCont<0)cCont=1;
    var cDate=h.indexOf('Date MAJ');if(cDate<0)cDate=2;
    var cStat=h.indexOf('Statut');if(cStat<0)cStat=3;
    var cAut=h.indexOf('Auteur');if(cAut<0)cAut=4;
    // Mise à jour si section existante
    if(obj.sectionOld){
      for(var i=1;i<d.length;i++){
        if(String(d[i][cSec]).trim()===String(obj.sectionOld).trim()){
          if(obj.section) sh.getRange(i+1,cSec+1).setValue(obj.section);
          sh.getRange(i+1,cCont+1).setValue(obj.contenu||'');
          sh.getRange(i+1,cDate+1).setValue(now);
          sh.getRange(i+1,cStat+1).setValue(obj.statut||'En vigueur');
          sh.getRange(i+1,cAut+1).setValue(obj.auteur||'Gérant');
          _setLastWrite();
          return{success:true,action:'updated'};
        }
      }
    }
    // Création nouvelle rubrique
    var row=sh.getLastRow()+1;
    sh.getRange(row,1,1,5).setValues([[obj.section||'',obj.contenu||'',now,obj.statut||'En vigueur',obj.auteur||'Gérant']]);
    _setLastWrite();
    return{success:true,action:'created'};
  }catch(e){Logger.log('savePMS: '+e.message);return{success:false,error:e.message};}
}

// ─── EXPORT CHARGES EXCEL — colonnes normalisées expert-comptable ──────────
// Retourne {success, url, name} — Google Sheets créé dans Drive
function exportChargesExcel(){
  try{
    var data=readSheet('Charges').filter(function(r){return r['Poste'];});
    if(!data.length) return{success:false,error:'Aucune charge à exporter'};
    var dFmt=Utilities.formatDate(new Date(),'Europe/Paris','yyyyMMdd_HHmm');
    var nss=SpreadsheetApp.create('MAISON41_Charges_'+dFmt);
    var sh=nss.getActiveSheet().setName('Charges');
    var moisLbl=['Jan','Fév','Mar','Avr','Mai','Juin','Juil','Août','Sep','Oct','Nov','Déc'];
    // Clés brutes dans le Sheets (sans accents)
    var moisKeys=['Jan','Fev','Mar','Avr','Mai','Juin','Juil','Aout','Sep','Oct','Nov','Dec'];
    var hdrs=['Poste','Type','Estimation €'].concat(moisLbl).concat(['Total Réel €','Écart €','Notes']);
    sh.appendRow(hdrs);
    var totalEstim=0,totalReel=0;
    data.forEach(function(c){
      var estim=parseFloat(c['Estimation EUR']||c['Estimé EUR']||c['Estimé']||c['Estimation']||0);
      totalEstim+=estim;
      var moisVals=moisKeys.map(function(m){return parseFloat(c[m]||c[m.replace('v','é').replace('u','û')]||0);});
      var realSum=moisVals.reduce(function(s,v){return s+v;},0);
      totalReel+=realSum;
      var ecart=realSum-estim;
      sh.appendRow([c['Poste']||'',c['Type']||'',estim].concat(moisVals).concat([realSum,ecart,c['Notes']||'']));
    });
    // Ligne totaux
    var totRow=data.length+2;
    sh.getRange(totRow,1).setValue('TOTAL');
    sh.getRange(totRow,3).setValue(totalEstim);
    sh.getRange(totRow,hdrs.length-1).setValue(totalReel);
    sh.getRange(totRow,hdrs.length).setValue(totalReel-totalEstim);
    sh.getRange(totRow,1,1,hdrs.length).setFontWeight('bold').setBackground('#f5f1e8');
    // Styles header
    sh.getRange(1,1,1,hdrs.length).setBackground('#0d0d0d').setFontColor('#f0c060').setFontWeight('bold');
    sh.setFrozenRows(1);sh.autoResizeColumns(1,hdrs.length);
    // Colorier colonne Écart (dernière - 1 = hdrs.length-1, 1-indexed = hdrs.length)
    var ecartColIdx=hdrs.length; // 1-indexed
    for(var i=2;i<=data.length+1;i++){
      var cell=sh.getRange(i,ecartColIdx);
      var v=cell.getValue();
      if(typeof v==='number') cell.setFontColor(v<0?'#0f4d28':v>0?'#c0392b':'#555');
    }
    Logger.log('exportChargesExcel: '+data.length+' lignes → '+nss.getName());
    return{success:true,url:nss.getUrl(),name:nss.getName(),lignes:data.length};
  }catch(e){Logger.log('exportChargesExcel: '+e.message);return{success:false,error:e.message};}
}
...
function setupSpreadsheet(){
  try{
    ...
    mkSheet('Recettes',['Ref.','Nom commercial','Gamme','Format principal','Rendement %','Batch kg MP','Duree bareme (min)','Statut','Nb ing.','Cout MP/kg','Date creation','Gluten','Oeufs','Lait','Sulfites','Moutarde','Autres allerg.','kcal/100g','Prot.','Lip.','Gluc.','Sel','Sync App'],3);
    mkSheet('Detail Recettes',['Ref. recette','Code MP','Designation MP','Qte par batch','Unite','Prix MP (EUR)','Cout ligne','% cout'],4);
    mkSheet('Fiches Techniques',['Ref.','Produit','Gluten','Crustaces','Oeufs','Poissons','Arachides','Lait','Fruits','Celeri','Moutarde','Sesame','Sulfites','Lupin','Mollusques','Traces','kcal','Prot','Lip','Gluc','Sel','T° bareme (°C)','DDM mois','N° agrement','Duree bareme (min)'],4);
